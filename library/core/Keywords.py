...
from library.core.ElementFinder import ElementFinder
from library.core.TestLogger import TestLogger
from library.core.utils import WebDriverCache, ConfigManager


def current_driver():
    assert isinstance(WebDriverCache.DriverCache.current_driver, webdriver.Remote)
    return WebDriverCache.DriverCache.current_driver
...
class Android(object):
    ...
    @staticmethod
    def get_network_state_code():
        return current_driver().network_connection

    @staticmethod
    @TestLogger.log()
    def open_app(server_url=ConfigManager.get_server_url(), desired_caps=ConfigManager.get_desired_caps(),
                 alis=None):
        """打开app"""
        WebDriverCache.DriverCache.open_app(server_url, desired_caps, alis)

    @staticmethod
    @TestLogger.log()
    def closed_app():
        WebDriverCache.DriverCache.close_app()

    @staticmethod
    @TestLogger.log()
    def launch_app():
        WebDriverCache.DriverCache.launch_app()

    @staticmethod
    @TestLogger.log()
    def closed_current_driver():
        WebDriverCache.DriverCache.quit_current()
    ...
    def get_last_verification_code():
        message = ElementFinder.find_elements((MobileBy.ANDROID_UIAUTOMATOR, 'new UiSelector().textContains("登录验证")'))
        if message.__len__():
